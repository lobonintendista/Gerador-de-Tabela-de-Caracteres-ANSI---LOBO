...
void exibirCabecalho() {
    printf("\n     =================================================\n");
    printf("        Gerador de Tabela de Caracteres ANSI - LOBO\n");
    printf("     =================================================\n\n");
}

int processarArquivo(const char *binFilename) {
    char tabela[256];  // Tabela a ser gerada
    ...
    for (unsigned long offset = offsetEntrada; offset <= offsetSaida; offset++) {
        ...
        if (fread(&byteLido, 1, 1, binFile) == 1) {
            ...
            if (isprint((unsigned char)byteLido)) {
                tabela[(unsigned char)byteLido] = byteLido;  // Mapeia o byte para o caractere correspondente
            }
        } else {
    ...
    for (int i = 0; i < 256; i++) {
        if (tabela[i] != '@') {
            fprintf(txtFile, "%02X=%c\n", i, tabela[i]); // Removido "00" antes do valor
        } else {
            fprintf(txtFile, "%02X=[%02X]\n", i, i); // Quando não há um caractere imprimível
        }
    }
